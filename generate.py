...
CONTENT = {
    ...
    "quality": {
        ...
        "scenario": [
            ...
            "本手册说明产品检验数维护、检验单基础配置与自动指派逻辑、PQC 检验作业及质量异常处置四大模块的系统操作步骤。"
        ],
        "steps": [
            {
                "title": "1. 产品检验数维护",
                "sub_steps": [
                    {
                        "id": "1.1", "name": "组装检验数维护",
                        "desc": "在【产品检验数维护】功能中，配置组装线的产品检验数量规则。组装检验数 = 大箱装入只数 × 检验系数。检验系数必须为 0.5 的倍数（如 0.5、1.0、1.5 等），由质量管理员根据产品特性和检验要求设定。例如大箱装入 100 只、检验系数 1.0，则检验数为 100。",
                        "remark": "检验系数需严格按 0.5 的倍数填写，系统会自动校验。系数越大抽样越多，检验精度越高但工作量也越大。",
                        "images": [{"img": "1.1组装检验数.png", "desc": "组装检验数维护"}]
                    },
                    {
                        "id": "1.2", "name": "注塑检验数维护",
                        "desc": "在【产品检验数维护】功能中，配置注塑机的产品检验数量规则。注塑检验数 = 装载数，即每模/每盘的实际装载数量。系统根据产品设定的装载数自动确定注塑产品的检验数量。",
