        ...
        "steps": [
            ...
            },
            {
                ...
                "sub_steps": [
                    ...
                    {
                        "id": "4.2", "name": "配置资源黑名单",
                        "desc": "【操作人员：质量管理员】<br><br>在 Web 后端维护资源黑名单（资源呼叫池），将以下人员排除，确保异常单不会指派给不恰当的人员：<br>• 休假中的人员<br>• 已离职的人员<br>• 出差中的人员<br>• 当下不可用的人员<br><br>系统在自动指派时会自动跳过黑名单中的人员。",
                        "remark": "资源配置支持添加、删除、修改，建议定期更新，确保人员信息的准确性。",
                        "images": []
                    },
                    {
                    ...
                    {
                        "id": "4.4", "name": "异常单自动指派",
                        "desc": "【执行方式：系统自动】<br><br>异常单发送成功后，系统根据以下规则自动指派：<br>• 根据不良率区间匹配对应的处理小组<br>• 自动跳过资源黑名单中的人员<br>• 指派给对应小组的可用人员<br><br>同时支持配置超时升级规则：当异常单在某个环节超时未处理时，系统自动升级至上级处理人，确保异常不被遗漏。",
                    ...
                    },
                    {
                        ...
                        "desc": "【操作人员：主管】<br><br>在「待审批」页签查看处置对策，判断处置结果是否合理：<br><br>✅ 若处置对策合理 → 点击【签核通过】→ 异常单流转至「待验收」状态<br>❌ 若处置对策不合理 → 点击【退回】→ 异常单回到「待处理」状态，被指派小组需重新填写对策",
                        "remark": "主管签核是确保处置对策合理性的关键环节。签核通过后不可再修改。",
                        "images": []
                    },
                    {
                        "id": "4.8", "name": "待验收 → 处置结果验证",
                        "desc": "【操作人员：第一群组】<br><br>在「待验收」页签收到处置完成通知后，对处置结果进行验证：<br><br>✅ 验证结果 OK → 填写验证耗时和验证说明 → 核准处置记录 → 异常单进入「已关闭」状态<br>❌ 验证结果 NG → 打回至「待处理」状态，被指派小组需重新录入对策",
                        "remark": "NG 打回后异常单回到「待处理」状态，被指派小组重新填写对策。",
                        "images": []
                    },
                    {
                        "id": "4.9", "name": "异常单结案",
