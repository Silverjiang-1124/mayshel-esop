                ...
                "sub_steps": [
                    {
                        ...
                        "desc": "质量管理员在 Web 后端配置异常处理小组。设定不良率区间与处理小组的对应关系（如不良率 0~5% → A组、5~10% → B组、10%以上 → C组），并指定各小组对应的检查群组和通知群组。当异常单发送时，系统根据不良率自动匹配对应的处理小组。",
                        "remark": "不良率区间不能重叠，系统会自动校验区间范围。一个异常处理小组可绑定多个通知群组。",
                        "images": []
                    },
                    {
                        "id": "4.2", "name": "配置资源黑名单",
                        "desc": "质量管理员在 Web 后端维护资源黑名单（资源呼叫池），将休假、离职、出差或当下不可用的人员排除，确保异常单不会指派给不恰当的人员。同时可配置各群组内可用的处理人员名单，系统在自动指派时会自动跳过黑名单中的人员。",
                        "remark": "资源配置支持添加、删除、修改，修改后即时生效。建议定期更新黑名单，确保人员信息的准确性。",
                        "images": []
                    },
                    {
                        "id": "4.3", "name": "发送异常单（前端作业平台）",
                        "desc": "现场作业人员在【作业人员工作平台】发现质量异常时，点击【发送异常】按钮，弹出异常单发送窗口。选择当前生产工单、设备编号，填写异常原因和不良数量。系统自动计算不良率，并根据配置的处理小组规则匹配对应的处理小组。确认后异常单发送成功，自动推送至对应小组待办列表。",
                        "remark": "异常原因支持从后端配置的异常原因库中选择，也可手动输入描述。不良率 = 不良数量 ÷ 生产总数 × 100%。",
                        "images": []
                    },
                    {
                        "id": "4.4", "name": "异常单自动指派",
                        "desc": "异常单发送成功后，系统根据配置的处理小组规则自动将异常单指派给对应的处理小组和人员，无需人工干预。同时系统支持配置超时升级规则：当异常单在某个环节超时未处理时，自动升级至上级处理人，确保异常不被遗漏。",
                        "remark": "自动指派基于不良率区间匹配处理小组规则。超时升级每触发一次自动记录升级日志。",
                        "images": []
                    },
                    {
                        "id": "4.5", "name": "未确认 → 异常单签收",
                        "desc": "被指派小组在 H5 端【异常处置】模块→「未确认」页签中查看系统自动指派的异常单。确认异常信息后进行签收，异常单状态变更为「待处理」，进入处置环节。支持手动调整指派给其他人员。",
                        "remark": "未确认页签仅显示新指派的异常单。签收后不可随意退回，确保处置责任到人。",
                        "images": []
                    },
                    {
                        "id": "4.6", "name": "待处理 → 处置对策录入",
                        "desc": "被指派小组在「待处理」页签查看异常单详情，在处置对策中填写具体的处理措施和方案，提交后进入待审批环节。",
                        "remark": "处置对策支持文字描述和附件上传。",
                        "images": []
                    },
                    {
                        "id": "4.7", "name": "待审批 → 主管签核",
                        "desc": "主管在「待审批」页签查看处置对策，判断处置结果是否合理。若 OK 则签核通过，异常单流转至「待验收」状态；若 NG 则退回，被指派小组需重新填写处置对策。",
                        "remark": "主管签核是确保处置对策合理性的关键环节。签核通过后不可再修改处置内容。",
                        "images": []
                    },
                    {
                        "id": "4.8", "name": "待验收 → 处置结果验证",
                        "desc": "第一群组在「待验收」页签收到处置完成通知后，对处置结果进行验证。输入验证耗时和验证说明。若验证结果 OK，则核准处置记录，异常单进入「已关闭」状态；若 NG，则打回至被指派小组重新处理。",
                        "remark": "NG 打回后异常单重新回到「待处理」状态，被指派小组需重新录入对策。",
                        "images": []
                    },
                    {
                        "id": "4.9", "name": "异常单结案",
                        "desc": "处置验证通过后，异常单状态变更为「已关闭」，处置流程结束。已关闭的异常单可在「已关闭」页签查询历史记录，包含完整的处置过程、对策、验证记录和耗时信息。",
